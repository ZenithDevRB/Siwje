...
-- Attack Loop (VERSIÓN SIMPLIFICADA Y COMPATIBLE)
        ...
        if enemy:IsA("Model") and enemy ~= character and enemy:FindFirstChild("Humanoid") then
            ...
            if enemyHum and enemyHum.Health > 0 and enemyRoot then
                ...
                if distance <= AttackRange then
                    ...
                    -- Método 1: Activar el bate (más compatible)
                    pcall(function()
                        bat:Activate()
                    end)
                    
                    -- Método 2: Daño directo (si el juego lo permite)
                    pcall(function()
                        enemyHum:TakeDamage(10)
                    end)
                    
                    -- Debug
                    print("Atacando a:", enemy.Name, "Distancia:", math.floor(distance))
                end
            end
        end
    end
